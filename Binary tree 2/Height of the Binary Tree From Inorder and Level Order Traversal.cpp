...
int heightOfTheTree(vector<int>& inorder, vector<int>& levelOrder, int N){
    map<int, int> mp; // Inorder Node value -> index in inorder array
    ...
    //1.root is the last element in inorder traversal then it has no right subtree
        if(mp[levelOrder[0]] == inorder.size()-1) 
            q.push({0, (int)inorder.size()-2, 1});
  //2.root is the first element in inorder traversal then it has no left subtree
        else if(mp[levelOrder[0]] == 0)
            q.push({1, (int)inorder.size()-1, 1});
        else
        {
            //has both left and right tree
       q.push({0, mp[levelOrder[0]]-1, 1});   // has left subtree
       q.push({mp[levelOrder[0]]+1, (int)inorder.size()-1, 1});//has right subtree
        }
    }
